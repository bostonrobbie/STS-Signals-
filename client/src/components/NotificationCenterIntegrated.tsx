...
/**
 * Notification Center Component - Integrated with WebSocket
 * Displays real-time notifications from WebSocket server
 */
...
export const NotificationCenterIntegrated: React.FC<
...
> = ({ userId, onNotificationReceived }) => {
  ...
  useEffect(() => {
    logger.info("Initializing notification center");
    ...
    try {
      ...
      const checkConnection = () => {
        setIsConnectedToWS(isConnected());
      };

      // Subscribe to notifications when connected
      socket.on("connect", () => {
        logger.info("WebSocket connected, subscribing to notifications");
        checkConnection();
        if (userId) {
      ...
      onTradeAlert((data: any) => {
        logger.info("Received trade alert", data);
        handleNewNotification({
          id: `trade_${Date.now()}`,
          type: "trade_alert",
          title: `Trade Alert: ${data.symbol}`,
          message: `${data.action} ${data.quantity} @ $${data.price}`,
          priority: "high",
          timestamp: new Date(),
          read: false,
          data,
      ...
      onStrategySignal((data: any) => {
        logger.info("Received strategy signal", data);
        handleNewNotification({
          id: `signal_${Date.now()}`,
          type: "strategy_signal",
          title: `Strategy Signal: ${data.strategyName}`,
          message: `${data.signal.toUpperCase()} signal (Strength: ${data.strength}%)`,
          priority: "normal",
          timestamp: new Date(),
          read: false,
          data,
      ...
      onPortfolioUpdate((data: any) => {
        logger.info("Received portfolio update", data);
        handleNewNotification({
          id: `portfolio_${Date.now()}`,
          type: "portfolio_update",
          title: "Portfolio Updated",
          message: `Return: ${data.totalReturn}% | Win Rate: ${data.winRate}%`,
          priority: "low",
          timestamp: new Date(),
          read: false,
          data,
      ...
      logger.error("Error initializing notification center", error);
    }
    // @ts-expect-error TS2454, TS2448
  }, [userId, handleNewNotification, dismissNotification]);
  ...
  const handleNewNotification = useCallback(
    ...
    // @ts-expect-error TS2454, TS2448
    [onNotificationReceived, dismissNotification]
  );
  ...
  return (
    <div className="relative">
      ...
      <button
        ...
        title={isConnectedToWS ? "Connected" : "Disconnected"}
      >
        <div className="relative">
